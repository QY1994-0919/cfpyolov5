...
# 计算类别的AP 
#没训练完成一个epoch 都要进行一个评估计算 就是利用ap_per_class函数进行的
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=()):
    ...
    # Sort by objectness
    i = np.argsort(-conf) #argsort将元素加负号后从小到大排序，conf大的元素在前面
    tp, conf, pred_cls = tp[i], conf[i], pred_cls[i]

    # Find unique classes 不一样的类别数 20 80
    unique_classes = np.unique(target_cls)
    nc = unique_classes.shape[0]  # number of classes, number of detections

    # Create Precision-Recall curve and compute AP for each class
    px, py = np.linspace(0, 1, 1000), []  # for plotting 绘图
    ap, p, r = np.zeros((nc, tp.shape[1])), np.zeros((nc, 1000)), np.zeros((nc, 1000)) #初始化约为0

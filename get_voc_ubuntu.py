...
def convert_annotation(year, image_id):
    ...
    out_file = open('voc/VOCdevkit/VOC%s/labels/%s.txt'%(year, image_id), 'w')
...
for year, image_set in sets:
    if not os.path.exists('voc/VOCdevkit/VOC%s/labels/'%(year)):
        os.makedirs('voc/VOCdevkit/VOC%s/labels/'%(year))
...
# 创建VOC文件夹及子文件夹
wd = os.getcwd()
data_base_dir = os.path.join(wd, "voc/VOC/")
...
for line in lines:
    line = line.replace('\n', '')
    if (os.path.exists(line)):
        shutil.copy(line, "voc/VOC/images/val") # 复制图片
        print('copying val img file  %s' % line + '\n')
        
    line = line.replace('JPEGImages', 'labels')
    line = line.replace('jpg', 'txt')
    if (os.path.exists(line)):
        shutil.copy(line, "voc/VOC/labels/val") # 复制label

...
from models.Functions import ConvBlock, Encoding, Mean, DropPath, Mlp, GroupNorm, LayerNormChannel


# 为same卷积或same池化自动扩充,即为零填充padding
...
class Conv(nn.Module):
    ...
    # 网络的执行顺序是根据 forward 函数来决定的（正向传播函数）
    def forward(self, x):  # 从里往外：conv bn act
        x = self.act(self.bn(self.conv(x)))
        return x

    def fuseforward(self, x):  # 从里往外：conv act
...
class BottleneckCSP(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, number, shortcut, groups, expansion
        ...
        c_ = int(c2 * e)  # hidden channels
        # conv标准模块组用Conv,基础conv调用nn.Conv2d
        self.cv1 = Conv(c1, c_, 1, 1)
        ...
        # *操作符可以把一个list拆开成一个个独立的元素,m相当于使用n次bottleneck操作
    ...
    def forward(self, x):
        ...
        y2 = self.cv2(x)
        x = self.cv4(self.act(self.bn(torch.cat((y1, y2), dim=1))))
        return x
...
class Focus(nn.Module):
    ...
    def forward(self, x):  # x(b,c,w,h) -> y(b,4c,w/2,h/2)
        # return self.conv(torch.cat([x[..., ::2, ::2], x[..., ::2, ::2],x[..., ::2, 1::2] , x[..., 1::2, 1::2]], 1))
        x = self.conv(torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))
        return x

###################################################EVCBlock#####################################################################
class LVCBlock(nn.Module):
    def __init__(self, c1, c2, num_codes):
        super(LVCBlock, self).__init__()
        self.c2 = c2
        self.num_codes = num_codes
        num_codes = 64

        self.conv_1 = ConvBlock(c1=c1, c2=c1, res_conv=True, stride=1)

        self.LVC = nn.Sequential(
            nn.Conv2d(c1, c1, 1, bias=False),
            nn.BatchNorm2d(c1),
            nn.ReLU(inplace=True),
            Encoding(c1=c1, num_codes=num_codes),
...
class EVCBlock(nn.Module):
    def __init__(self, c1, c2, channel_ratio=4, base_channel=16):
        ...
        expansion = 2
        ch = c2 * expansion
        # Stem stage: get the feature maps by conv block (copied form resnet.py) 进入框架之前的处理
        ...
        self.lmlp = LightMLPBlock(c1, c2, act_layer=nn.GELU, mlp_ratio=4., drop=0.,
...
# 用于第二级分类 ：将YOLOv5检测后的结果再进行分类（车牌识别，）
class Classify(nn.Module):
    # Classification head, i.e. x(b,c1,20,20) to x(b,c2)
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        super(Classify, self).__init__()
        self.aap = nn.AdaptiveAvgPool2d(1)  # to x(b,c1,1,1) 自适应平均赤化操作
